...
$HENRIK_API_KEY = $config['HENRIK_API_KEY'];
...
function fetchHenrikApi($url, $apiKey) {
    $opts = [
        'http' => [
            'method' => 'GET',
...
// Parâmetros
$game = $_GET['game'] ?? 'lol'; // 'lol' ou 'valorant'
$action = $_GET['action'] ?? '';
...
if ($game === 'valorant') {
    ...
    switch ($action) {
        case 'account':
            ...
            $url = "https://api.henrikdev.xyz/valorant/v1/account/" . urlencode($name) . "/" . urlencode($tag);
            break;
        ...
        case 'mmr':
            ...
            $url = "https://api.henrikdev.xyz/valorant/v2/mmr/{$region}/" . urlencode($name) . "/" . urlencode($tag);
            break;
        ...
        case 'matches':
            ...
            $size = $_GET['size'] ?? 5;
            $url = "https://api.henrikdev.xyz/valorant/v3/matches/{$region}/" . urlencode($name) . "/" . urlencode($tag) . "?size={$size}";
            break;
            
        default:
            echo json_encode(['error' => true, 'message' => 'Ação inválida para Valorant']);
            exit;
    }
    
    $result = fetchHenrikApi($url, $HENRIK_API_KEY);
...
switch ($action) {
    ...
    case 'ranked':
        ...
        $url = "https://{$platform}/lol/league/v4/entries/by-puuid/{$puuid}";
        break;
...
$result = fetchRiotApi($url, $RIOT_API_KEY);
echo json_encode($result);
